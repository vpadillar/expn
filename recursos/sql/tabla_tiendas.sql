...
ALTER FUNCTION tabla_tienda(integer, text, integer, integer)
  ...
  select * from usuario_ciudad
  select case when length(celular) !=0 then celular else '0' end from usuario_tienda
  select t.nit,c.nombre,t.nombre,t.direccion,t.fijo,t.celular,c.id 
  from usuario_tienda as t inner join usuario_ciudad as c on(t.ciudad_id = c.id and c.status=true and t.status =true)
  select * from usuario_tienda order by id