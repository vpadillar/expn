...
begin
	...
	SELECT COALESCE(array_to_json(array_agg(row_to_json(p))), '[]') from (
		...
		case when p.entregado then 1 else 0 end as estado,p.id from pedido_pedido as p 
		inner join usuario_empleado as e1 on (p.alistador_id=e1.usuario_ptr_id and p.empresa_id=1) 
