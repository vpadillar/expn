-- Function: reactivar_pedido(integer)

-- DROP FUNCTION reactivar_pedido(integer);

CREATE OR REPLACE FUNCTION reactivar_pedido(id_ped integer)
  RETURNS json AS
$BODY$
declare 
	pedido record;
...
begin
	...
	if pedido.id is not null then
		update pedido_pedido set rectivacion=true where id = id_ped;
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION reactivar_pedido(integer)
  OWNER TO postgres;
